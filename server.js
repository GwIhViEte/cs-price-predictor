        ...
        if (response.data.code === 200) {
            ...
            if (response.data.data && Array.isArray(response.data.data)) {
                response.data.data.forEach(item => {
                    items.push({
                        id: item.id,
                        name: item.value, // 注意：这里是 value，不是 name
...
// 获取热门饰品API - 使用正确的热门系列接口
app.get('/api/hot-items', async (req, res) => {
    try {
        await checkRateLimit();
        
        // 使用热门系列饰品列表接口 - 注意这是POST请求
...
app.get('/api/chart-data/:itemId', async (req, res) => {
    try {
        ...
        // 获取单件饰品图表数据
        const response = await axios.get(`${CSQAQ_CONFIG.baseURL}/goods/${itemId}/chart`, {
        ...
        if (response.data.code === 200) {
            ...
            res.status(400).json({
                success: false,
                error: response.data.msg || '获取图表数据失败'
            });
        }
...
- 市场统计数据: ${itemInfo.statistic}`;
        ...
        // 构建专业的分析师prompt
        const prompt = `你是一位顶级的CS饰品市场数据分析师。请根据以下提供的 **当前实时数据** 和 **官方7日历史均价**，对饰品进行一次全面、深入的投资分析。
...
请注意：在最近的更新中添加了交易撤回功能，现在卖饰品的钱要在平台冻结7天！
...
2. **平台选择建议**: 哪个平台的当前售价远低于7日均价，可能存在购买机会？

3. **短期（1-3天）价格预测**: 综合所有信息，预测接下来的短期价格走势（例如：可能会向7日均价回归，或继续偏离），并给出核心理由。

4. **中期（4-7天）价格预测**: 综合所有信息，预测接下来的中期价格走势（例如：可能会向7日均价回归，或继续偏离），并给出核心理由。

5. **长期（8天以上）价格预测**: 综合所有信息，预测接下来的长期价格走势（例如：可能会向7日均价回归，或继续偏离），并给出核心理由。
